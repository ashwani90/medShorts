...
const swiper = new Swiper('.swiper-container', {
  ...
  function addNewsItemsToSwiper(items) {
    ...
    // Remove older slides if the total number exceeds 20
    if (swiper.slides.length > 20) {
      const slidesToRemove = swiper.slides.length - 20;
      for (let i = 0; i < slidesToRemove; i++) {
        swiper.removeSlide(0); // Remove the oldest slide (first slide)
      }
    }
  }
...
Use CSS transitions to make slide removal smoother.
